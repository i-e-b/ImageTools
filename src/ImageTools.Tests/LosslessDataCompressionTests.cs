...
// LZW -> 165.13kb; 147.76kb
        ...
        public void info__n_grams_of_file()
        {
            ...
            foreach (var b in data)
            {
                ...
                var minGram = MinRot(nGram, n); // deal with rotations, by finding minimum valued rotation.
            ...
            {
                readOut.AppendLine($"    #{c} -> {kvp.Key.ToString(fmt)} x {kvp.Value}");
                countTop += kvp.Value;
                if (c++ >= topTake) break;
            }
            
            var sizeInTop = countTop * n;
        ...
        /// </summary>
        private static long MinRot(long n, int w)
        {
            if (w < 2) return n;
