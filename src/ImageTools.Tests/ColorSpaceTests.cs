...
namespace ImageTools.Tests
{
    ...
    public class ColorSpaceTests {
        ...
        public void XYZ__separations()
        {
            ...
            BitmapTools.XYZPlanes_To_ArgbImage(dst, 0, Xp, zeroP, zeroP);
            dst.SaveBmp("./outputs/3_XYZ_X-only.bmp");
                    
            BitmapTools.XYZPlanes_To_ArgbImage(dst, 0, zeroP, Yp, zeroP);
            dst.SaveBmp("./outputs/3_XYZ_Y-only.bmp");
                    
            BitmapTools.XYZPlanes_To_ArgbImage(dst, 0, zeroP, zeroP, Zp);
            dst.SaveBmp("./outputs/3_XYZ_Z-only.bmp");
        }
        
        [Test, Description("show the result of just one plane at a time from an image")]
        public void LMS__separations()
        {
            using var bmp = Load.FromFile("./inputs/3.png");
            using var dst = new Bitmap(bmp);
            BitmapTools.ArgbImageToXYZPlanes(bmp, out var Xp, out var Yp, out var Zp);
            BitmapTools.TranslatePlanes(ColorSpace.XYZ_To_LMS, Xp, Yp, Zp, out var Lp, out var Mp, out var Sp);
            var zeroP = new double[Xp.Length]; // to zero out other planes
            for (int i = 0; i < zeroP.Length; i++) { zeroP[i] = 0.0; }

            BitmapTools.TranslatePlanes(ColorSpace.LMS_To_XYZ, Lp, zeroP, zeroP, out Xp, out Yp, out Zp);
            BitmapTools.XYZPlanes_To_ArgbImage(dst, 0, Xp, Yp, Zp);
            dst.SaveBmp("./outputs/3_LMS_L-only.bmp");
                    
            BitmapTools.TranslatePlanes(ColorSpace.LMS_To_XYZ, zeroP, Mp, zeroP, out Xp, out Yp, out Zp);
            BitmapTools.XYZPlanes_To_ArgbImage(dst, 0, Xp, Yp, Zp);
            dst.SaveBmp("./outputs/3_LMS_M-only.bmp");
                    
            BitmapTools.TranslatePlanes(ColorSpace.LMS_To_XYZ, zeroP, zeroP, Sp, out Xp, out Yp, out Zp);
            BitmapTools.XYZPlanes_To_ArgbImage(dst, 0, Xp, Yp, Zp);
            dst.SaveBmp("./outputs/3_LMS_S-only.bmp");
        }
        ...
        public void Oklab__separations()
        {
            ...
            BitmapTools.PlanesToImage(dst, ColorSpace.Oklab_To_LinearRGB_Byte, 0, Lp, Ap, Bp); // use this to get a representation of the gamma correction
            dst.SaveBmp("./outputs/3_Oklab_gamma_shift.bmp");
