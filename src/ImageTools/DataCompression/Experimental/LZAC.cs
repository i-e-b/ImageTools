...
namespace ImageTools.DataCompression.Experimental
{
    ...
    public class LZAC
    {
        ...
        private const int TerminationSymbol = 289;

        public LZAC()
        {
            _model = new ProbabilityModels.LearningModel(289, 2);
            ...
            while (true)
            {
                ...
                if (matchLength > 0)
                {
                    ...
                    else // insert a back-reference
                    {
                        ...
                        for (int i = 0; i < matchLength; i++) { WriteToWindowAndAdvance(window[matchLeftSide + i]); }
